// build.rs
//
// Downloads the Microsoft Cognitive Services Speech SDK NuGet package
// and generates Rust FFI bindings via bindgen.
//
// 移植自 jabber-tools/cognitive-services-speech-sdk-rs 的 build.rs，
// SDK 版本升级至 1.50.0（截至 2026-06 NuGet 最新），后续需要时可调整。
...
const SPEECH_SDK_VERSION: &str = "1.50.0";
...
fn main() {
    ...
    let nuget_package_url = format!(
        "https://www.nuget.org/api/v2/package/Microsoft.CognitiveServices.Speech/{SPEECH_SDK_VERSION}"
    );
    ...
    #[cfg(feature = "bindgen-gen")]
    {
        ...
        if !bindings_path.exists() {
            ...
            let bindings = bindgen::Builder::default()
                ...
                // 包装层期望扁平枚举常量名（如 ResultReason_NoMatch），
                // 关闭枚举名前缀以匹配上游手写代码的命名约定。
                .prepend_enum_name(false)
                // 关闭布局断言：SDK 含不透明/空结构体（_azac_empty 等），
                // bindgen 的 size 断言会在编译期 panic。
                .layout_tests(false)
                .generate()
...
fn main() {
    ...
    let mac_sdk_url = format!(
        "https://csspeechstorage.blob.core.windows.net/drop/{SPEECH_SDK_VERSION}/MicrosoftCognitiveServicesSpeech-MacOSXCFramework-{SPEECH_SDK_VERSION}.zip"
    );
    ...
    if renew {
        Command::new("unzip")
            .args(["-o", sdk_zip_file.to_str().unwrap(), "-d", sdk_output_dir.to_str().unwrap()])
            .status()
            .unwrap();
    }
