...
impl SpeechConfig {
    ...
    /// Set the voice used for speech synthesis (e.g. "zh-CN-XiaoxiaoNeural").
    pub fn set_speech_synthesis_voice_name(&mut self, voice: &str) -> Result<()> {
        self.properties.set_property(PropertyId::SpeechServiceConnectionSynthVoice, voice)
    }

    pub fn get_speech_synthesis_voice_name(&self) -> Result<String> {
        self.properties.get_property(PropertyId::SpeechServiceConnectionSynthVoice, "")
    }

    /// Set the language used for speech synthesis (e.g. "zh-CN").
    pub fn set_speech_synthesis_language(&mut self, lang: &str) -> Result<()> {
        self.properties.set_property(PropertyId::SpeechServiceConnectionSynthLanguage, lang)
    }
