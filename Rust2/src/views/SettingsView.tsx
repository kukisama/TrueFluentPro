...
import {
  ...
  Sun, Moon, Shield, Zap, Loader2, ChevronRight,
...
const VENDOR_BADGE: Record<string, { label: string; badge: string; glyph: string; color: string }> = {
  azure_open_ai:          { label: "Azure OpenAI",  badge: "AZ", glyph: "☁", color: "blue" },
  api_management_gateway: { label: "APIM 网关",     badge: "AP", glyph: "⇆", color: "purple" },
  open_ai_compatible:     { label: "OpenAI 兼容",   badge: "OA", glyph: "✦", color: "green" },
  azure_speech:           { label: "Azure Speech",   badge: "SP", glyph: "🎤", color: "yellow" },
  azure_translator:       { label: "Azure Translator",badge: "TR", glyph: "🌐", color: "blue" },
  deep_l:                 { label: "DeepL",          badge: "DL", glyph: "D",  color: "blue" },
  tencent_cloud:          { label: "腾讯云",         badge: "TC", glyph: "T",  color: "blue" },
  alibaba_cloud:          { label: "阿里云",         badge: "AL", glyph: "A",  color: "orange" },
  custom:                 { label: "自定义",         badge: "CU", glyph: "⚙",  color: "gray" },
};
    ...
    try {
      ...
      setTestReports((r) => ({
        ...
        [id]: {
          endpoint_id: id,
          endpoint_name: "",
          items: [{ model_id: "-", capability: "-", status: "failed" as const, summary: String(e), duration_ms: 0 }],
          duration_ms: 0,
        },
      }));
    } finally {
                  ...
                  {report && (
                    <div className="mt-2 pt-2 border-t border-[var(--border-subtle)]">
                      ...
                      >
                        ...
                        <TestReportSummaryBadges items={report.items} />
                        <span className="ml-auto text-[var(--text-muted)]">{report.duration_ms}ms</span>
                      </button>
                      {isExpanded && (
...
function TestResultRow({ item }: { item: EndpointTestItem }) {
  ...
  const icon =
    item.status === "success" ? <CheckCircle2 size={13} className="text-emerald-500 shrink-0" /> :
    item.status === "failed" ? <XCircle size={13} className="text-red-400 shrink-0" /> :
    <SkipForward size={13} className="text-[var(--text-muted)] shrink-0" />;

  return (
    <div className="rounded-md bg-[var(--surface-1)] px-2.5 py-1.5">
      <button className="flex items-center gap-2 w-full text-left" onClick={() => setExpanded(!expanded)}>
        ...
        <span className="text-[10px] text-[var(--text-muted)] shrink-0">{item.model_id} · {item.duration_ms}ms</span>
        {item.detail && <ChevronRight size={10} className={cn("text-[var(--text-muted)] transition-transform", expanded && "rotate-90")} />}
      </button>
      {expanded && item.detail && (
        <div className="mt-1.5 pl-5 space-y-1">
          ...
          {item.request_url && (
            <p className="text-[10px] text-[var(--text-muted)] break-all">URL: {item.request_url}</p>
          )}
        </div>
      )}
    </div>
...
function EndpointForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <GlassCard className="mb-4 border-brand-500/20" glow>
      <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">新建端点</h3>
      ...
      {isSpeech ? (
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4 mt-4">
            <div>
              <Label>API 版本</Label>
              <Input value={form.api_version} onChange={(e) => update("api_version", e.target.value)} placeholder={activeProfile?.default_api_version || "无"} />
            </div>
            <div>
              <Label>认证方式</Label>
...
function EndpointEditPanel({ endpoint, onClose }: { endpoint: AiEndpoint; onClose: () => void }) {
  ...
  const handleSave = async () => {
    const updated: AiEndpoint = { ...form, models: isSpeech ? [] : models };
    ...
    <div className="space-y-3">
      {isSpeech ? (
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div><Label>名称</Label><Input value={form.name} onChange={(e) => update("name", e.target.value)} /></div>
            <div><Label>区域</Label><Input value={form.speech_region} onChange={(e) => update("speech_region", e.target.value)} placeholder="southeastasia" /></div>
          </div>
          <div><Label>Speech 终结点</Label><Input value={form.speech_endpoint} onChange={(e) => update("speech_endpoint", e.target.value)} placeholder="https://region.api.cognitive.microsoft.com" /></div>
          <div><Label>订阅密钥</Label><Input type="password" value={form.speech_subscription_key} onChange={(e) => update("speech_subscription_key", e.target.value)} /></div>
        </div>
      ) : (
        /* ═══ AI 编辑表单 ═══ */
        <>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>名称</Label><Input value={form.name} onChange={(e) => update("name", e.target.value)} /></div>
            <div><Label>端点 URL</Label><Input value={form.url} onChange={(e) => update("url", e.target.value)} /></div>
            <div><Label>API Key</Label><Input type="password" value={form.api_key} onChange={(e) => update("api_key", e.target.value)} /></div>
            <div><Label>API 版本</Label><Input value={form.api_version || ""} onChange={(e) => update("api_version", e.target.value)} /></div>
            <div><Label>区域</Label><Input value={form.region || ""} onChange={(e) => update("region", e.target.value)} /></div>
            <div>
              <Label>认证方式</Label>
              <Select className="w-full" value={form.auth_header_mode} onChange={(e) => update("auth_header_mode", e.target.value)}>
                <option value="auto">自动</option>
      ...
      <div className="flex items-center justify-between pt-2">
        ...
        <div className="flex gap-2">
          <Button variant="secondary" size="sm" onClick={onClose}><X size={14} /> 取消</Button>
          <Button size="sm" onClick={handleSave}><Check size={14} /> 保存</Button>
        </div>
      </div>
    </div>
  );
