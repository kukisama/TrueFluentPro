//! PR-1.9~1.18: 任务监控命令模块
//! 对标 C# TaskQueueMonitorViewModel + AudioTaskQueueService
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  数据结构 — 对齐 C# BatchBucketNavItem + TaskQueueItemViewModel
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  纯函数 — 对齐 C# TaskQueueItemViewModel.StageDisplayName / StageColor / StatusDisplayName
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  Bucket 定义 — 对齐 C# TaskQueueMonitorViewModel.Buckets
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
fn bucket_key_to_status(key: &str) -> &str {
    match key {
        ...
        "running" => "Executing",
        "completed" => "Completed",
        "failed" => "Failed,Timeout,Interrupted", // 多状态，storage 层解析
        "cancelled" => "Cancelled",
        _ => "Queued",
    }
}
...
use crate::storage::MonitorTaskRow;

/// S-5: 提取 MonitorTaskRow → MonitorTaskItem 映射为公共函数，消除 3 处重复
...
pub async fn monitor_get_snapshot(
    ...
    let _sort_col = sort_column.unwrap_or_else(|| "SubmittedAt".into());
    let _sort_asc = sort_ascending.unwrap_or(false);
    ...
    let tasks = db
        .monitor_get_tasks_by_status(status_filter, &_sort_col, _sort_asc)
        .await
        .map_err(|e| e.to_string())?;

    // 解析为前端 DTO
    let task_items: Vec<MonitorTaskItem> = tasks
        .into_iter()
        .map(map_task_row)
        .collect();
...
/// PR-4.2: 批量删除（仅允许 completed/cancelled/failed/timeout）
#[tauri::command]
...
/// M-3: RFC 4180 CSV 字段转义 — 双引号包裹，内部双引号翻倍
