...
pub async fn audiolab_import_files(
    ...
    for path_str in &paths {
        ...
        // Duration (简单估算; 真实实现需要解析音频头)
        // 对 wav 文件: file_size / (sample_rate * channels * bytes_per_sample)
        // 对 mp3: 近似用 file_size / bitrate
        // PR-1 使用简单估算，PR-2 会用真正的音频解码
        ...
        // 创建关联 session（session_type='audio'）
...
fn estimate_audio_duration_ms(path: &Path, file_size_bytes: i64) -> i64 {
    ...
    match ext.as_str() {
        "wav" => {
            // WAV: 通常 PCM 16bit mono 16kHz → 32000 bytes/sec
            let bytes_per_sec = 16000 * 1 * 2; // sample_rate * channels * 2 bytes
        ...
        "mp3" => {
            // MP3: 假设 128kbps → 16000 bytes/sec
...
pub async fn audiolab_get_bundle(
    ...
    // 找到该 session 关联的 audio_file
    // 通过 studio_sessions.source_asset_id 直接获取 file_id
...
pub async fn audiolab_start_transcription(
    ...
    // 创建 studio_task 记录（复用 session_tasks 体系）
    let task = StudioTask {
        id: task_id.clone(),
...
/// 打开音频文件用于播放：保证 mp3 路径可用（若源文件非 mp3 则当前直接返回源路径，真正转码在后续 PR）
#[tauri::command]
pub async fn audiolab_playback_open(
    state: State<'_, AppState>,
    session_id: String,
) -> Result<AudioPlaybackInfo, String> {
    // 通过 session.source_asset_id 找到关联 audio_file
    let session = state.db.studio_get_session(&session_id).await.map_err(|e| e.to_string())?
        .ok_or_else(|| format!("找不到 session: {}", session_id))?;
    let file = if let Some(ref asset_id) = session.source_asset_id {
        state.db.audiolab_get_file(asset_id).await.map_err(|e| e.to_string())?
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  PR-3: 阶段生成 + AutoTags + 研究
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// 启动阶段生成任务（Summary / Insight / MindMap / Podcast / Research / Translation / Custom）
#[tauri::command]
pub async fn audiolab_start_stage(
    ...
    // 创建 stage_output 记录（status=Processing）
...
pub async fn audiolab_start_research(
    ...
    let task = StudioTask {
        id: task_id.clone(),
        session_id: topic_id.clone(), // 用 topic_id 关联
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  PR-4: 说话人重命名 + 段落编辑 + 导出 + 实时桥接
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

/// 重命名说话人（该 speaker_index 全部段更新）
#[tauri::command]
...
pub async fn audiolab_import_from_realtime(
    ...
    // 查找实时翻译会话的录音文件和段落
    // 策略：通过 realtime_session_id 查找 translation_sessions 表，获取对应录音路径
    // 然后导入 audio_files + 复制已有 segments
    // TODO: 需要访问 translation_sessions 表和对应的录音文件路径
    // 当前返回占位，待实时翻译模块数据表确定后补充
