...
export function TaskMonitorView() {
  ...
  const selectedTask = useMemo(
    () => snapshot?.current_bucket_tasks.find(tk => tk.id === selectedTaskId) ?? null,
  ...
  const handleSelectAll = useCallback(() => {
    if (!snapshot) return;
    setSelectedIds(new Set(snapshot.current_bucket_tasks.map(tk => tk.id)));
  }, [snapshot]);
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (searchText.trim()) {
      ...
      tasks = tasks.filter(tk => tk.id.toLowerCase().includes(q) || tk.audio_file_name.toLowerCase().includes(q) || tk.stage_display_name.toLowerCase().includes(q));
    }
    if (stageFilter.length > 0) {
      tasks = tasks.filter(tk => stageFilter.includes(tk.stage));
    }
    return tasks;
                    ...
                    {executions.map(exec => (
                      <div key={exec.id}>
                        <div onClick={() => {
                          ...
                          selectionLockedRef.current = newId !== null;
                        }}
