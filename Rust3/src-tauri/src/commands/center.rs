...
use tfp_core::{
    ...
    CanvasRound, StudioTask, StudioAsset, ImageGenRequest,
...
pub async fn center_start_video_round(
    ...
    tokio::spawn(async move {
        ...
        for poll_i in 0..max_polls {
            ...
            match provider.poll_status(&video_id, &request.endpoint_id).await {
                Ok(result) => {
        ...
        match super::studio::studio_download_video(&dl_url, &output_path).await {
            Ok(path_str) => {
